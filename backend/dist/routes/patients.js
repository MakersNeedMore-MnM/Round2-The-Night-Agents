...
exports.patientsRouter.get('/', (req, res) => {
    res.json({ success: true, patients: database_1.db.patients });
});
exports.patientsRouter.get('/:id', (req, res) => {
    const patient = database_1.db.patients.find((p) => p.id === req.params.id);
    if (!patient) {
        return res.status(404).json({ success: false, message: 'Patient not found' });
    }
    const encounter = database_1.db.encounters.find((e) => e.patientId === patient.id);
    res.json({ success: true, patient, encounter });
});
exports.patientsRouter.post('/identify', (req, res) => {
    const { identifierType, identifierValue, newPatientData } = req.body;
    let patient;
    if (identifierType === 'ABHA') {
        patient = database_1.db.patients.find((p) => p.abhaId.replace(/[^0-9]/g, '') === (identifierValue || '').replace(/[^0-9]/g, ''));
    }
    else if (identifierType === 'UHID') {
        patient = database_1.db.patients.find((p) => p.uhid.toLowerCase() === (identifierValue || '').toLowerCase());
    }
    else if (identifierType === 'MOBILE') {
        patient = database_1.db.patients.find((p) => p.phone.replace(/[^0-9]/g, '').includes((identifierValue || '').replace(/[^0-9]/g, '')));
    }
        ...
        database_1.db.patients.push(patient);
        // Create encounter
        database_1.db.encounters.push({
            id: `enc_${newId}`,
            patientId: newId,
    ...
    if (!patient) {
        patient = database_1.db.patients[0];
    }
    const encounter = database_1.db.encounters.find((e) => e.patientId === patient?.id);
    res.json({
        success: true,
...
exports.patientsRouter.post('/:id/consent', (req, res) => {
    const patient = database_1.db.patients.find((p) => p.id === req.params.id);
    if (!patient) {
        return res.status(404).json({ success: false, message: 'Patient not found' });
    }
