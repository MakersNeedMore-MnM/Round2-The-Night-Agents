...
exports.adminRouter.get('/opd-command-center', (req, res) => {
    const totalEncounters = database_1.db.encounters.length;
    const completed = database_1.db.encounters.filter((e) => e.status === 'COMPLETED' || e.status === 'READY_FOR_CONSULTATION').length;
    const activeAlerts = database_1.db.triageAlerts.filter((a) => a.status === 'ACTIVE').length;
    const totalDocs = database_1.db.documents.length;
    res.json({
        success: true,
        isDemoData: true,
...
exports.adminRouter.get('/audit-logs', (req, res) => {
    res.json({
        success: true,
        auditLogs: database_1.db.auditLogs,
    });
});
