export interface RedFlagAssessment {
  ...
  urgencyTitleEn: string;
  urgencyTitleHi: string;
  patientGuidanceEn: string;
  patientGuidanceHi: string;
  triageReason: string;
  detectedSymptoms: string[];
  shouldHaltIntake: boolean;
  staffAlertNeeded: boolean;
}
...
export function evaluateRedFlags(
  ...
  const hasDyspnea = associated.some((s: string) => s.toLowerCase().includes('breathless') || s.toLowerCase().includes('dyspnea'));
  const hasDiaphoresis = associated.some((s: string) => s.toLowerCase().includes('sweat') || s.toLowerCase().includes('cold'));
  const hasDangerousRadiation = radiation.some((r: string) => r.toLowerCase().includes('arm') || r.toLowerCase().includes('jaw'));

  if (isChestPain && (hasDyspnea || hasDiaphoresis || (hasDangerousRadiation && severity >= 7))) {
    if (hasDyspnea) detected.push('Chest pain with acute breathlessness (Dyspnea)');
    if (hasDiaphoresis) detected.push('Cold diaphoresis reported');
    if (hasDangerousRadiation) detected.push('Radiation to left arm / cervical dermatome');
