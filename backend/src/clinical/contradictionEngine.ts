...
export function detectContradictions(
  ...
  const reportedNoAllergy = 
    (typeof currentAllergies === 'string' && currentAllergies.toLowerCase().includes('no known')) ||
    (Array.isArray(currentAllergies) && currentAllergies.some((a: any) => typeof a === 'string' && a.toLowerCase().includes('no known')));
  ...
  const reportedNoChronic = 
    (typeof currentConditions === 'string' && currentConditions.toLowerCase().includes('no known')) ||
    (Array.isArray(currentConditions) && currentConditions.some((c: any) => typeof c === 'string' && c.toLowerCase().includes('no known')));
