import { CHEST_PAIN_PATHWAY, AYUSH_AYURVEDA_PATHWAY, ClinicalQuestion } from '../clinical/ontology';
...
export class AiProviderService {
   ...
   * Speech-To-Text (ASR)
   */
  async transcribeSpeech(audioData: string | Buffer, language: string = 'hi'): Promise<{
    transcript: string;
   ...
   * Text-To-Speech (TTS)
   */
  async synthesizeSpeech(text: string, language: string = 'hi'): Promise<{
    audioUrl: string;
    format: string;
  }> {
   ...
   */
  async extractClinicalEntities(rawInput: string, language: string = 'en'): Promise<{
   ...
   */
  async classifyDocument(fileContentOrName: string): Promise<{
    docType: 'PRESCRIPTION' | 'LAB_REPORT' | 'DISCHARGE_SUMMARY' | 'RADIOLOGY' | 'OPD_SLIP';
    confidence: number;
    facilityGuess: string;
  }> {
   ...
   */
  async ocrDocument(fileName: string, fileType: string): Promise<{
...
Advice: Monitor BP and blood glucose weekly.`,
  ...
  getNextClinicalQuestion(
    ...
    const pathway = pathwayKey === 'AYUSH_AYURVEDA' ? AYUSH_AYURVEDA_PATHWAY : CHEST_PAIN_PATHWAY;
